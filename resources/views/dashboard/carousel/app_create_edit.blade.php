                ...
                <fieldset>
                    <div class="form-group">
                        <div class="col-xs-6">
                            ...
                            <el-upload class="avatar-uploader" action="/upload_image" :show-file-list="false" :on-success="imageUrlAndroidHandle">
                                <img v-if="imageUrlAndroid" :src="imageUrlAndroid" class="el-avatar">
                                <i v-else class="el-icon-plus avatar-uploader-icon"></i>
                            </el-upload>
                            <el-input :value="imageUrlAndroid" type="hidden" name="carousel[android_icon]"></el-input>
                            <el-button class="btn-danger" size="medium" @click="clearImageUrlAndroid">清空图片</el-button>
                        </div>

                        <div class="col-xs-6">
                            <label>{{ 'iOS端图片:' }}</label><br>
                            <el-upload class="avatar-uploader" action="/upload_image" :show-file-list="false" :on-success="imageUrlIosHandle">
                                <img v-if="imageUrlIos" :src="imageUrlIos" class="el-avatar">
                                <i v-else class="el-icon-plus avatar-uploader-icon"></i>
                            </el-upload>
                            <el-input :value="imageUrlIos" placeholder="请输入内容" type="hidden" name="carousel[ios_icon]"></el-input>
                    ...
                    <div class="form-group">
                        ...
                        <input type="hidden" class="form-control" :value="version" name="carousel[version]">
                    </div>
        ...
        new Vue({
            ...
            data: function () {
                return {
                    ...
                    version:"{{ isset($carousel) ? ($carousel->version) : (Input::old('carousel')['version']) }}",
                    start_version:"{{ isset($carousel) ? $carousel->start_version : Input::old('carousel[start_version]') }}",
                    end_version:"{{ isset($carousel) ? $carousel->end_version : Input::old('carousel[end_version]') }}"
                };
            },
            methods: {
                //Android端图片
