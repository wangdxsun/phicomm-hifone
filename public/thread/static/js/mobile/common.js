...
var page = {
	converthtml : function() {
		...
		var selector = '';
		if(lastpage) {
			selector += '<a id="select_a" class="pages">';
	...
	open : function(pop, type, url) {
		...
		if(typeof pop == 'string') {
			...
			if(type == 'alert') {
				...
				pop = '<div class="tip"><dt>'+ pop +'</dt>' +
                    '<a href="javascript:;" onclick="popup.close();"><dd class="btn-center">确定</dd></a></div>'
                ...
                pop = '<div class="tip"><dt>'+ pop +'</dt>' +
                    '<a href="'+ url +'"><dd class="btn-left">确定</dd></a>' +
                    '<a href="javascript:;" onclick="popup.close();"><dd class="btn-right">取消</dd></a>' +
                    '</div>'
            }
...
function topicadminform_submit() {
	$.ajax({
		type:'POST',
		url:$("#topicadminform").attr('action'),
		data:$("#topicadminform").serialize(),
		dataType:'xml'
	}).success(function(s) {
		popup.open(s.lastChild.firstChild.nodeValue);
		evalscript(s.lastChild.firstChild.nodeValue);
...
function moderateform_submit() {
	$.ajax({
		type:'POST',
		url:$("#moderateform").attr('action'),
		data:$("#moderateform").serialize(),
		dataType:'xml'
	}).success(function(s) {
		popup.open(s.lastChild.firstChild.nodeValue);
		evalscript(s.lastChild.firstChild.nodeValue);
